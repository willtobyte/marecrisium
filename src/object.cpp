...
namespace {
  ...
  int object_gc(lua_State* state) {
    ...
    luaL_unref(state, LUA_REGISTRYINDEX, proxy->on_mouse_down);
    proxy->on_mouse_down = LUA_NOREF;
...
objectproxy::objectproxy(entt::registry& registry, entt::entity entity, std::string_view name, std::string_view kind, int environment)
  ...
  lua_getfield(L, -1, "on_mouse_down");
  if (lua_isfunction(L, -1))
    on_mouse_down = luaL_ref(L, LUA_REGISTRYINDEX);
  else
    lua_pop(L, 1);
