...
void particle::draw(float camera_x, float camera_y) const noexcept {
  ...
  for (auto i = 0uz; i < n; ++i) {
    ...
    if (life <= .0f) {
      constexpr SDL_FColor transparent = {1.f, 1.f, 1.f, .0f};
      vx[0] = {{.0f, .0f}, transparent, {.0f, .0f}};
      vx[1] = {{.0f, .0f}, transparent, {1.f, .0f}};
      vx[2] = {{.0f, .0f}, transparent, {1.f, 1.f}};
      vx[3] = {{.0f, .0f}, transparent, {.0f, 1.f}};
      continue;
    }
