...
class MimallocConan(ConanFile):
    name = "mimalloc"
    description = (
        "mimalloc v3.3.2 packaged for marecrisium. "
        "Temporary local copy of the conan-center recipe pinned to 3.3.2 — the upstream version "
        "that enables static override of malloc/free on Windows with the static CRT (no DLL). "
        "Drop this directory once https://github.com/conan-io/conan-center-index/pull/30174 is merged."
    )
    license = "MIT"
    ...
    def validate(self):
        ...
        if (
            self.options.override
            and self.options.get_safe("single_object")
            and self.options.get_safe("inject")
        ):
            raise ConanInvalidConfiguration(
    ...
    def generate(self):
        ...
        tc.variables["MI_WIN_REDIRECT"] = (
            "ON" if self.options.get_safe("win_redirect") else "OFF"
        )
    ...
    def package_info(self):
        ...
        self.cpp_info.set_property(
            "cmake_target_name",
            "mimalloc" if self.options.shared else "mimalloc-static",
        )
